...
function actionIndex($params = null) {
    ...
    if (isUser()) {
        ...
        $in_cart = sql_query("SELECT `bs`.*, `cat`.* FROM `basket` `bs` INNER JOIN `catalog` `cat` ON `cat`.`id` = `bs`.`product_id` WHERE `bs`.`cookie_id` = '" . $customer_id . "' AND status = 0");
    }
...
function actionCheckout() {    
    if(AJAX) {
        ...
        if (isUser()) {
            $user_id = getUserId();
            sql_updateOne('basket', " user_id = '" . $user_id . "', status = '1'", " cookie_id = '" . $customer_id . "'");
        } else {
            sql_updateOne('basket', " status = '1'", " cookie_id = '" . $customer_id . "'");
        }      
        echo 'OK';
        exit();
    }       
...
function actionDelete() {
    if(AJAX) {
        $cookie_id = mysqli_real_escape_string(getDb(), strip_tags(stripslashes($_POST['user_id'])));
        $status = mysqli_real_escape_string(getDb(), strip_tags(stripslashes($_POST['status'])));
        if(isAdmin()) {
            sql_delete('basket', "cookie_id = '".$cookie_id."' AND status = '".$status."'");
            echo 'Удалено!';
        } else {
            echo 'Действие не разрешено!';
        }        
    }
    exit();
}